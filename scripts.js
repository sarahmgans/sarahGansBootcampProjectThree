...
// Define the scroll function that will be used to scroll from the landing page to the beginning of the form, from the submit button to the results, and from the reset button back to the beginning of the form. Add 'scroll' as a function on the 'myApple' object. 
...
// Attach an event listener onto the start link on the landing page, and on click, have the page scroll to the beginning of the form using the scroll function from above. Add this event listener as a function on the 'myApple' object. 
myApple.clickEventLink = () => {
...
// Attach an event listener onto the submit input on the form. Add this event listener as well as a function on the 'myApple' object. 
myApple.clickEventSubmit = () => {
    $('form').on('submit', function (e) {
        ...
        // Declare a variable to be an array called myApple.appleChoices. This will be filled with eight apple objects, all of which will have the same use.
        const appleChoices = myApple.appleProperties[use];

        // Declare a variable to be an array called myApple.finalChoice. This will be filled with the final apple object. 
        const finalChoice = [];

        // Create a for loop that loops through the myApple.appleChoices array and matches the property/value pairs of an apple object with the users checked values of the texture, flavour and size inputs to produce one final object. This final object will be stored in the finalChoice array. 
        ...
        // Store the name of this final object in the myApple.finalChoice array in a variable called myApple.finalApple. Save the url property/value pair of each of the apples in a variable called myApple.appleInfo. 
...
// Attach an event listener to the reset input on the form, so that on click, the html that was posted to the ‘result’ section of the page is cleared, the radio buttons are unchecked and the form is scrolled back to the top of the page, using once again the scroll function from above. Add this event listener as a function on the 'myApple' object. 
...
// Define the init function that will be holding the script. Inside the init function, the three event listener functions that were defined above will be called, and within each of them, the scroll function that was also defined above will be called. 
myApple.init = () => {
    myApple.clickEventLink(myApple.scroll);
    myApple.clickEventSubmit(myApple.scroll);
    myApple.clickEventReset(myApple.scroll);
}
